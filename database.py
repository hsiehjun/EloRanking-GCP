...
class PostgresDatabase:
    ...
    def get_faction_matchup_prediction(self, f1: str, f2: str) -> Dict[str, Any]:
        """Calculates head-to-head empirical matchup stats, predicted win odds, and recent clashes between two factions."""
        f1_clean = f1.strip()
        f2_clean = f2.strip()

        # 1. Macro meta stats for both factions
        meta_stats = self.get_faction_meta_stats()
        factions_meta = {f["faction"].lower(): f for f in meta_stats.get("factions", [])}

        f1_meta = factions_meta.get(f1_clean.lower(), {
            "faction": f1_clean, "win_rate": 50.0, "tier": "B", "avg_score": 70.0, "total_matches": 0
        })
        f2_meta = factions_meta.get(f2_clean.lower(), {
            "faction": f2_clean, "win_rate": 50.0, "tier": "B", "avg_score": 70.0, "total_matches": 0
        })

        with self.get_connection() as conn:
            with conn.cursor(cursor_factory=extras.RealDictCursor) as cursor:
                # 2. Query head-to-head matches between f1 and f2
                cursor.execute("""
                SELECT 
                    m.id, m.event_id, e.name as event_name, m.round, m.match_date,
                    m.player1_id, m.player1_name, m.player1_faction, m.player1_score,
                    m.player2_id, m.player2_name, m.player2_faction, m.player2_score,
                    m.winner_id, m.loser_id, m.is_draw,
                    CASE WHEN m.player1_faction ILIKE %s THEN 1 ELSE 2 END as f1_pos
                FROM matches m
                LEFT JOIN events e ON m.event_id = e.id
                WHERE (
                    (m.player1_faction ILIKE %s AND m.player2_faction ILIKE %s)
                    OR
                    (m.player1_faction ILIKE %s AND m.player2_faction ILIKE %s)
                )
                AND m.is_done = TRUE AND is_bye = FALSE
                ORDER BY COALESCE(m.match_date, e.event_date) DESC NULLS LAST, m.round DESC
                LIMIT 50;
                """, (
                    f"%{f1_clean}%",
                    f"%{f1_clean}%", f"%{f2_clean}%",
                    f"%{f2_clean}%", f"%{f1_clean}%"
                ))
                clashes = [dict(r) for r in cursor.fetchall()]

                # 3. Best and hardest matchups for f1
                f1_details = self.get_faction_details(f1_clean, limit=1)
                f1_matchups = f1_details.get("matchups", [])
                qual_f1 = [m for m in f1_matchups if m.get("total_matches", 0) >= 3] or f1_matchups
                f1_prey = max(qual_f1, key=lambda x: (x.get("win_rate", 0), x.get("total_matches", 0)), default=None)
                f1_nemesis = min(qual_f1, key=lambda x: (x.get("win_rate", 100), -x.get("total_matches", 0)), default=None)

                # Best and hardest matchups for f2
                f2_details = self.get_faction_details(f2_clean, limit=1)
                f2_matchups = f2_details.get("matchups", [])
                qual_f2 = [m for m in f2_matchups if m.get("total_matches", 0) >= 3] or f2_matchups
                f2_prey = max(qual_f2, key=lambda x: (x.get("win_rate", 0), x.get("total_matches", 0)), default=None)
                f2_nemesis = min(qual_f2, key=lambda x: (x.get("win_rate", 100), -x.get("total_matches", 0)), default=None)

        total_h2h = len(clashes)
        f1_wins = 0
        f2_wins = 0
        draws = 0
        f1_scores = []
        ...
        formatted_clashes = []
        for c in clashes:
            is_f1_side1 = (c.get("f1_pos") == 1)
            p1_id = c.get("player1_id")
            p2_id = c.get("player2_id")
            winner_id = c.get("winner_id")
            is_draw = bool(c.get("is_draw"))

            s1 = c.get("player1_score")
            s2 = c.get("player2_score")
            f1_s = s1 if is_f1_side1 else s2
            f2_s = s2 if is_f1_side1 else s1

            if f1_s is not None: f1_scores.append(float(f1_s))
            if f2_s is not None: f2_scores.append(float(f2_s))

            if is_draw:
                draws += 1
                winner_side = "draw"
            elif (is_f1_side1 and winner_id == p1_id) or (not is_f1_side1 and winner_id == p2_id):
                f1_wins += 1
                winner_side = "f1"
            else:
                f2_wins += 1
            ...
            formatted_clashes.append({
                "id": c.get("id"),
                "event_id": c.get("event_id"),
                "event_name": c.get("event_name") or "Tournament",
                "round": c.get("round") or 1,
                "match_date": str(c.get("match_date") or "")[:10],
                "f1_player_id": p1_id if is_f1_side1 else p2_id,
                "f1_player_name": c.get("player1_name") if is_f1_side1 else c.get("player2_name"),
                "f1_score": f1_s,
                "f2_player_id": p2_id if is_f1_side1 else p1_id,
                "f2_player_name": c.get("player2_name") if is_f1_side1 else c.get("player1_name"),
                "f2_score": f2_s,
                "winner_side": winner_side
            })

        f1_actual_wr = round((f1_wins * 100.0 / total_h2h), 1) if total_h2h > 0 else 50.0
        f2_actual_wr = round((f2_wins * 100.0 / total_h2h), 1) if total_h2h > 0 else 50.0

        f1_avg_score = round(sum(f1_scores) / len(f1_scores), 1) if f1_scores else float(f1_meta.get("avg_score") or 70.0)
        f2_avg_score = round(sum(f2_scores) / len(f2_scores), 1) if f2_scores else float(f2_meta.get("avg_score") or 70.0)

        # 4. Bayesian Predicted Win Probability
        w1 = float(f1_meta.get("win_rate") or 50.0)
        w2 = float(f2_meta.get("win_rate") or 50.0)
        diff_wr = w1 - w2
        macro_prob_1 = 50.0 + (diff_wr * 1.5)
        macro_prob_1 = max(15.0, min(85.0, macro_prob_1))

        if total_h2h > 0:
            k = 10.0
            pred_f1 = (k * macro_prob_1 + total_h2h * f1_actual_wr) / (k + total_h2h)
        else:
            pred_f1 = macro_prob_1

        pred_f1 = round(max(10.0, min(90.0, pred_f1)), 1)
        pred_f2 = round(100.0 - pred_f1, 1)

        return {
            "f1": {
                "name": f1_clean,
                "win_rate": w1,
                "tier": f1_meta.get("tier", "B"),
                "avg_score": f1_avg_score,
                "total_matches": f1_meta.get("total_matches", 0),
                "spotlight_prey": f1_prey,
                "spotlight_nemesis": f1_nemesis
            },
            "f2": {
                "name": f2_clean,
                "win_rate": w2,
                "tier": f2_meta.get("tier", "B"),
                "avg_score": f2_avg_score,
                "total_matches": f2_meta.get("total_matches", 0),
                "spotlight_prey": f2_prey,
                "spotlight_nemesis": f2_nemesis
            },
            "prediction": {
                "f1_win_prob": pred_f1,
                "f2_win_prob": pred_f2,
                "favorite": f1_clean if pred_f1 >= 50.0 else f2_clean,
                "advantage_pts": round(abs(pred_f1 - pred_f2), 1)
            },
            "head_to_head": {
                "total_games": total_h2h,
                "f1_wins": f1_wins,
                "f2_wins": f2_wins,
                "draws": draws,
                "f1_actual_win_rate": f1_actual_wr,
                "f2_actual_win_rate": f2_actual_wr,
                "f1_avg_score": f1_avg_score,
                "f2_avg_score": f2_avg_score,
                "score_differential": round(f1_avg_score - f2_avg_score, 1)
            },
            "clashes": formatted_clashes[:15]
        }
