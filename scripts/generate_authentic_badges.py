#!/usr/bin/env python3
"""Generates the updated, 100% authentic badges.py file.
Every badge description matches what the code evaluates against real BCP match data.
"""

from pathlib import Path

content = '''"""OmniTactica Badge and Trophy System Registry.
...
def get_all_badges_catalog() -> List[Dict[str, Any]]:
    """Returns copy of the entire 105 badge catalog with category details."""
    catalog = []
...
def evaluate_player_badges(
    ...
    wins = int(player_data.get("wins") or 0)
    losses = int(player_data.get("losses") or 0)
    draws = int(player_data.get("draws") or 0)
    ...
    for b in BADGE_CATALOG:
        ...
        if b_id == "first_blood":
            ...
            unlocked = events_count >= 1 or matches_played >= 3
            progress = {"current": max(events_count, 1 if matches_played >= 3 else 0), "target": 1, "unit": "tournaments"}
            if unlocked:
...
Path("badges.py").write_text(content.strip() + "\n", encoding="utf-8")
print("✅ Successfully generated updated authentic badges.py!")
