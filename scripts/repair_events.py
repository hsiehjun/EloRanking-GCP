...
WHERE raw_json IS NOT NULL
  AND (
      ...
      OR ((raw_json->>'numberOfRounds') ~ '^[0-9]+$' AND (raw_json->>'numberOfRounds')::int > COALESCE(num_rounds, 0))
      OR ((raw_json->>'numRounds') ~ '^[0-9]+$' AND (raw_json->>'numRounds')::int > COALESCE(num_rounds, 0))
  );
"""
