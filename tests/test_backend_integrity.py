...
def test_route_parity():
    """Verify all 185 API routes match the canonical specification."""
    ...
    route_paths = {r[1] for r in registered_routes}
    assert "/api/auth/verify-login-2fa" in route_paths, "Missing /api/auth/verify-login-2fa route"
    assert "/api/auth/resend-login-2fa" in route_paths, "Missing /api/auth/resend-login-2fa route"
