...
class WahapediaService:
    ...
    def get_stratagems_for_detachment(self, faction: str, detachment: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if "STARSHATTER" in det_upper or "RELENTLESS" in det_upper:
            det_strats = [
                {"name": "Merciless Reclamation", "type": "Detachment (Battle Tactic)", "cp": 1, "phase": "Shooting or Fight Phase", "description": "Target one friendly Necrons unit (excluding Monsters and Titanic). Attacks targeting enemy units within range of an objective marker add +1 to the Wound roll."},
                {"name": "Unyielding Forms", "type": "Detachment (Battle Tactic)", "cp": 1, "phase": "Opponent's Shooting/Fight Phase", "description": "Target one friendly Necrons Vehicle or Mounted unit (excluding Titanic) targeted by attacks. If the incoming attack's Strength is greater than the unit's Toughness, subtract 1 from the Wound roll."},
                {"name": "Chronoshift", "type": "Detachment (Strategic Ploy)", "cp": 1, "phase": "Movement Phase", "description": "Target one friendly Necrons Vehicle or Mounted unit (excluding Titanic) that Advances. Do not roll for the Advance; add a flat 6\" to its Move characteristic."},
                {"name": "Dimensional Tunnel", "type": "Detachment (Strategic Ploy)", "cp": 1, "phase": "Movement Phase", "description": "Target one friendly Necrons Vehicle or Mounted unit (excluding Titanic). Models in this unit can move through models and terrain features as if they were not there."},
                {"name": "Endless Servitude", "type": "Detachment (Strategic Ploy)", "cp": 1, "phase": "End of Opponent's Fight Phase", "description": "Target one friendly Necrons unit (excluding Monsters and Titanic) within range of an objective marker you control. That unit immediately activates its Reanimation Protocols."},
                {"name": "Reactive Reposition", "type": "Detachment (Strategic Ploy)", "cp": 1, "phase": "Opponent's Shooting Phase", "description": "Target one friendly Necrons Vehicle or Mounted unit (excluding Titanic) after an enemy unit selects it as a target. That unit makes a Normal move of up to D6\" (or 6\")."}
            ]
