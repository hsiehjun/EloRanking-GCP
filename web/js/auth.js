...
function switchBcpModalTab(tab) {
  const btnPwd = document.getElementById('bcp-tab-btn-pwd');
  const btnToken = document.getElementById('bcp-tab-btn-token');
  const formPwd = document.getElementById('bcp-form-credentials');
  const formToken = document.getElementById('bcp-form-token');

  if (tab === 'token') {
    if (btnPwd) { btnPwd.className = 'btn btn-outline'; }
    if (btnToken) { btnToken.className = 'btn btn-primary'; }
    if (formPwd) formPwd.style.display = 'none';
    if (formToken) formToken.style.display = 'block';
  } else {
    if (btnPwd) { btnPwd.className = 'btn btn-primary'; }
    if (btnToken) { btnToken.className = 'btn btn-outline'; }
    if (formPwd) formPwd.style.display = 'block';
    if (formToken) formToken.style.display = 'none';
  }
}

function copyBcpSnippet() {
  const snippet = document.getElementById('bcp-console-snippet');
  if (snippet) {
    snippet.select();
    navigator.clipboard.writeText(snippet.value).then(() => {
      alert("✅ 1-Line BCP snippet copied! Open your browser console on bestcoastpairings.com, paste, and press Enter, then paste the output back here.");
    }).catch(() => {
      alert("Copy snippet: " + snippet.value);
    });
  }
}

async function handleConnectBcpTokenDirect(e) {
  if (e) e.preventDefault();
  const rawInput = document.getElementById('bcp-link-raw-token');
  const errorDiv = document.getElementById('bcp-token-error');
  const submitBtn = document.getElementById('bcp-token-submit-btn');

  const rawVal = rawInput ? rawInput.value.trim() : '';
  if (!rawVal) {
    if (errorDiv) {
      errorDiv.innerText = 'Please paste your BCP token or JSON payload.';
      errorDiv.style.display = 'block';
    }
    return;
  }

  let accessToken = rawVal;
  let refreshToken = '';
  ...
  if (submitBtn) {
    submitBtn.disabled = true;
    submitBtn.innerText = 'Validating & Linking Token...';
  }
  if (errorDiv) errorDiv.style.display = 'none';

  try {
    const res = await window.api.connectBcpToken(accessToken, refreshToken);
    if (res && res.success) {
      if (res.user) {
        currentUser = res.user;
        localStorage.setItem('native_user_profile', JSON.stringify(currentUser));
      ...
      if (errorDiv) {
        errorDiv.innerText = res.error || 'Failed to validate BCP token.';
        errorDiv.style.display = 'block';
      }
    }
  } catch (err) {
    ...
    if (submitBtn) {
      submitBtn.disabled = false;
      submitBtn.innerText = '🔗 Link Browser Session Token';
    }
  }
}
