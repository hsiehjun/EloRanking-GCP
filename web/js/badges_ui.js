...
(function(window) {
  ...
  function showCelebrationModal(data, newlyUnlocked, isIncremental, userKey, userId, newlyUnlockedIds, batchHash) {
    ...
    var showcaseHtml = showcaseItems.slice(0, 4).map(function(b) {
      return [
        '<div style="flex: 1 1 125px; min-width: 115px; padding: 0.65rem 0.5rem; background: rgba(15, 23, 42, 0.7); border-radius: var(--radius-sm); border: 1px solid ' + (b.border || 'var(--border)') + '; text-align: center; box-sizing: border-box;">',
        '  <div style="font-size: 1.5rem; margin-bottom: 0.25rem;">' + (b.icon || '⚔️') + '</div>',
        '  <div style="font-size: 0.76rem; font-weight: 700; color: #fff; margin-bottom: 0.25rem; line-height: 1.25; word-break: break-word;">' + escapeHtml(b.name) + '</div>',
    ...
    modal.innerHTML = [
      '<div class="modal-card" style="max-width: 520px; width: calc(100% - 1.25rem); margin: 0 auto; background: #0f172a; border-radius: var(--radius-lg); border: 1.5px solid rgba(245, 158, 11, 0.7); box-shadow: 0 10px 40px rgba(0,0,0,0.8), 0 0 30px rgba(245, 158, 11, 0.25); overflow-x: hidden; max-height: 90vh; overflow-y: auto; box-sizing: border-box;">',
      '  <div style="padding: 2rem 1.15rem 1.15rem; text-align: center; background: radial-gradient(circle at top, rgba(245, 158, 11, 0.25), transparent 75%); border-bottom: 1px solid rgba(255,255,255,0.08); position: relative;">',
      '    <button type="button" class="modal-close" onclick="window.BadgesUI.closeCelebrationModal(\'' + escapeHtml(userKey) + '\', \'' + escapeHtml(userId || '') + '\', ' + idsJson + ', \'' + escapeHtml(batchHash) + '\')" style="position: absolute; top: 0.55rem; right: 0.65rem; width: 32px; height: 32px; display: inline-flex; align-items: center; justify-content: center; border-radius: 50%; background: rgba(15, 23, 42, 0.75); border: 1px solid rgba(148, 163, 184, 0.25); font-size: 1.05rem; color: #cbd5e1; cursor: pointer; z-index: 2;">✕</button>',
      '    <div style="display: inline-flex; align-items: center; justify-content: center; gap: 0.35rem; font-size: 0.68rem; font-weight: 800; color: #fbbf24; text-transform: uppercase; letter-spacing: 0.06em; background: rgba(245, 158, 11, 0.15); padding: 0.25rem 0.7rem; border-radius: 9999px; border: 1px solid rgba(245, 158, 11, 0.35); margin-bottom: 0.65rem; max-width: calc(100% - 2.5rem);">' + headerTag + '</div>',
      '    <h2 style="font-size: 1.35rem; font-weight: 800; color: #fff; margin: 0 0 0.4rem; letter-spacing: -0.02em;">' + headerTitle + '</h2>',
      '    <div style="font-size: 0.84rem; color: #cbd5e1; line-height: 1.45; max-width: 440px; margin: 0 auto;">' + headerSub + '</div>',
      '    <div style="display: flex; align-items: stretch; justify-content: center; gap: 0.5rem; margin-top: 0.95rem;">',
      '      <div style="flex: 1; min-width: 0; padding: 0.5rem 0.35rem; background: rgba(255,255,255,0.04); border-radius: var(--radius-sm); border: 1px solid rgba(255,255,255,0.08); text-align: center;">',
      '        <div style="font-size: 1.2rem; font-weight: 800; color: #fbbf24; font-family: var(--font-mono);">' + (isIncremental ? ('+' + newlyUnlocked.length) : totalBadgeCount) + '</div>',
      '        <div style="font-size: 0.62rem; color: #94a3b8; text-transform: uppercase; font-weight: 700; line-height: 1.2;">' + (isIncremental ? 'New Honors' : 'Honors Unlocked') + '</div>',
      '      </div>',
      '      <div style="flex: 1; min-width: 0; padding: 0.5rem 0.35rem; background: rgba(255,255,255,0.04); border-radius: var(--radius-sm); border: 1px solid rgba(255,255,255,0.08); text-align: center;">',
      '        <div style="font-size: 1.2rem; font-weight: 800; color: #38bdf8; font-family: var(--font-mono);">' + glory.toLocaleString() + '</div>',
      '        <div style="font-size: 0.62rem; color: #94a3b8; text-transform: uppercase; font-weight: 700; line-height: 1.2;">Glory Requisition</div>',
      '      </div>',
      '      <div style="flex: 1; min-width: 0; padding: 0.5rem 0.35rem; background: rgba(255,255,255,0.04); border-radius: var(--radius-sm); border: 1px solid rgba(255,255,255,0.08); text-align: center;">',
      '        <div style="font-size: 1.2rem; font-weight: 800; color: #a855f7;">' + (rank.icon || '🛡️') + '</div>',
      '        <div style="font-size: 0.64rem; color: #cbd5e1; font-weight: 700; line-height: 1.2;">' + escapeHtml(rank.title) + '</div>',
      '      </div>',
      '    </div>',
      '  </div>',
      '  <div style="padding: 1.1rem 1.15rem;">',
      '    <div style="font-size: 0.72rem; font-weight: 700; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 0.6rem;">' + (isIncremental ? 'Unlocked Battlefield Honors:' : 'Top Signature Honors:') + '</div>',
      '    <div style="display: flex; gap: 0.55rem; margin-bottom: 0.5rem; flex-wrap: wrap;">' + showcaseHtml + '</div>',
      moreCountText,
      '    <div style="display: flex; gap: 0.65rem; margin-top: 1.1rem;">',
      '      <button type="button" class="btn btn-primary" style="flex: 1; padding: 0.65rem 0.85rem; font-weight: 700; font-size: 0.88rem;" onclick="window.BadgesUI.closeCelebrationModal(\'' + escapeHtml(userKey) + '\', \'' + escapeHtml(userId || '') + '\', ' + idsJson + ', \'' + escapeHtml(batchHash) + '\'); if (typeof switchHubSubtab === \'function\') switchHubSubtab(\'trophies\');">Inspect Trophy Room</button>',
      '      <button type="button" class="btn btn-secondary" style="padding: 0.65rem 0.9rem; font-size: 0.88rem;" onclick="window.BadgesUI.closeCelebrationModal(\'' + escapeHtml(userKey) + '\', \'' + escapeHtml(userId || '') + '\', ' + idsJson + ', \'' + escapeHtml(batchHash) + '\')">' + (isIncremental ? 'Acknowledge' : 'Continue') + '</button>',
      '    </div>',
      '  </div>',
      '</div>'
    ].join('\n');
